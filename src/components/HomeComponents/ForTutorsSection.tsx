...
export default function ForTutorsSection() {
  ...
  return (
    <section className="py-24 lg:py-36 px-4 transition-colors duration-300 bg-slate-50 dark:bg-slate-950 overflow-hidden">
      <div className="container mx-auto">
        <div className="grid lg:grid-cols-2 gap-16 items-center">
          ...
          <div className="order-2 lg:order-1 relative">
            {/* Background Glow */}
            <div className="absolute -top-10 -left-10 w-40 h-40 bg-green-200 dark:bg-green-600/20 rounded-full blur-[80px] opacity-40" />
            
            <div className="relative z-10">
              <div className="relative rounded-3xl overflow-hidden shadow-2xl border-[12px] border-white dark:border-slate-900">
                <Image
                  width={600}
                  height={750}
              ...
              {/* Floating Growth Stat Card */}
              <div className="absolute top-10 -right-6 lg:-right-10 bg-white/90 dark:bg-slate-800/90 backdrop-blur-md p-6 rounded-2xl shadow-xl border border-white/20 dark:border-slate-700 hidden md:block">
                <div className="flex items-center gap-3 mb-2">
                  <div className="bg-green-500 p-2 rounded-lg">
                    <TrendingUp className="text-white w-5 h-5" />
                  </div>
                  <span className="text-sm font-semibold text-gray-500 dark:text-slate-400">Monthly Growth</span>
                </div>
              ...
              <div className="absolute -bottom-6 left-10 bg-white dark:bg-slate-800 p-4 rounded-xl shadow-lg border border-gray-50 dark:border-slate-700 flex items-center gap-3">
                <div className="relative w-10 h-10 rounded-full overflow-hidden border-2 border-green-500">
                  <div className="bg-gray-200 dark:bg-slate-700 w-full h-full flex items-center justify-center">
                    <Users className="w-5 h-5 text-gray-400 dark:text-slate-500" />
                  </div>
                </div>
                <div>
                  <div className="flex items-center gap-1">
                    <span className="text-sm font-bold text-gray-900 dark:text-white">Top Rated Tutor</span>
                    <CheckCircle className="w-3 h-3 text-blue-500 fill-blue-500" />
                  </div>
                  <p className="text-xs text-gray-500 dark:text-slate-400">5.0 (480 Reviews)</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="order-1 lg:order-2">
            ...
            <h2 className="text-4xl lg:text-5xl font-extrabold text-gray-900 dark:text-white mb-6 leading-[1.15]">
              Transform Your Expertise into a <span className="text-green-600 dark:text-green-400">Thriving Business</span>
            </h2>
            ...
            <div className="mt-12 flex flex-col sm:flex-row items-center gap-6">
              <Link href={"/become-a-tutor"}>
                <Button className="w-full sm:w-auto hover:cursor-pointer bg-green-600 hover:bg-green-700 dark:bg-green-600 dark:hover:bg-green-500 text-white px-10 py-7 text-lg rounded-2xl shadow-xl shadow-green-100 dark:shadow-none transition-all hover:-translate-y-1 active:scale-95">
                  Apply to Teach
                </Button>
              </Link>
              <Link href="/how-it-works" className="text-gray-600 dark:text-slate-300 font-semibold hover:text-green-600 dark:hover:text-green-400 transition-colors underline decoration-2 underline-offset-4">
                Learn how it works
              </Link>
            </div>
          </div>
